...
function needCLodop() {
  try {
    ...
    let verOPR = ua.match(/OPR\D?\d+/i) as string[];
    let verFF = ua.match(/Firefox\D?\d+/i) as string[];
    const x64 = ua.match(/x64/i);
    if ((!verTrident) && (!verIE) && (x64)) return true;
    if (verFF) {
      verFF = verFF[0].match(/\d+/) as string[];
      if ((Number(verFF[0]) >= 41) || (x64)) return true;
    } else if (verOPR) {
      verOPR = verOPR[0].match(/\d+/) as string[];
      if (Number(verOPR[0]) >= 32) return true;
    } else if ((!verTrident) && (!verIE)) {
      let verChrome = ua.match(/Chrome\D?\d+/i) as string[];
      if (verChrome) {
        verChrome = verChrome[0].match(/\d+/) as string[];
        if (Number(verChrome[0]) >= 41) return true;
      }
    }
    return false;
  } catch (err) {
...
export default function getLodop(oOBJECT?: {
  parentNode: { removeChild: (arg0) => void; };
} | undefined, oEMBED?: { parentNode: { removeChild: (arg0) => void; }; } | undefined) {
  ...
  const strCLodopInstall1 = `<br><font color='#FF00FF'>Web打印服务CLodop未安装启动，点击这里
  <a href='CLodop_Setup_for_Win32NT.zip' target='_self'>下载32位</a>、<a href='CLodop_Setup_for_Win64NT_4.142EN.zip' target='_self'>下载64位</a>
  执行安装`;
  ...
  try {
    ...
    if (needCLodop()) {
      ...
      if (!LODOP) {
        ...
        if (oWrap) oWrap.innerHTML = strCLodopInstall1 + (CLodopIsLocal ? strCLodopInstall2 : '') + strCLodopInstall3 + oWrap.innerHTML;
        return null;
      }
