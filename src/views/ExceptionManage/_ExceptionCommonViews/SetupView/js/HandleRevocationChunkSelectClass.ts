...
export class HandleRevocationChunkSelectClass {
  ...
  constructor(data: IExceptionPlate) {
    this.PlateDetail = data;
    this.ChunkList = [];
  }

  /** 多个ID使用半角逗号（，）隔开，最多可填写10个订单 */
  async getChunkList(orderIDString: string) {
    if (!orderIDString) {
      MpMessage.error('查询失败', '请输入订单ID');
      return;
    }
    if (!/^\d{1,2}-\d{5,10}(,\d{1,2}-\d{5,10})*$/.test(orderIDString)) {
      MpMessage.error('查询失败', '订单ID输入格式不正确');
      return;
    }
    const list = orderIDString.split(',');
    ...
    if (this._lastOrderIDString === orderIDString && this.ChunkList.length > 0) { // 重复请求
      return;
    }
    ...
    if (resp?.data.isSuccess) {
      ...
      this._lastOrderIDString = orderIDString;
