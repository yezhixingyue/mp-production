...
import { IInstanceAttribute } from '@/views/productionManagePages/ProcessDecompositionOrderList/types/types';
...
const props = defineProps<{
  ...
  getMaterialList:(TypeID: string) => Promise<void>
}>();
...
const EpCascaderValue = computed({
  ...
  set([ClassID, TypeID]: ['' | number, string]) {
    ...
    // 此处获取数据 -- 向原有列表中追加
    if (condition.TypeID) {
      props.getMaterialList(condition.TypeID);
    }
    radio.value = '';
  },
});
...
const onOpen = async () => {
  ...
  if (props.Attribute.Material.ID && props.Attribute.Material.TypeID && props.FactoryMaterialClassList.length === 0) {
    await props.getClassList();
    await props.getMaterialList(props.Attribute.Material.TypeID);
  }

  if (props.FactoryMaterialClassList.length > 0) {
    if (radio.value) {
      const TypeID = props.Attribute.Material.TypeID || props.MaterialList.find(it => it.ID === radio.value)?.TypeID;
      if (TypeID) {
        const target = props.FactoryMaterialClassList.find(lv1 => lv1.MaterialTypes.some(lv2 => lv2.TypeID === TypeID));
        if (target) {
