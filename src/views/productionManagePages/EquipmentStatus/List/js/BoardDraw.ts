...
export class BoardDraw {
  ...
  private _drawTitle(
    ...
    const _className = getLimitWords(it.Equipment.ClassName, 14, this.layout.TitleLeftWidth - 10, this.canvas || undefined);
    this.ctx.fillText(_className, x + 8 + 0.5, y + 6 + 2 + 0.5, this.layout.TitleLeftWidth);

    this.ctx.font = 'bold 14px YaHei';
    const _groupName = getLimitWords(it.Equipment.GroupName, 14, this.layout.TitleLeftWidth - 10, this.canvas || undefined);
    this.ctx.fillText(_groupName, x + 8 + 0.5, y + 6 + 22 + 0.5, this.layout.TitleLeftWidth);

    // 3 右侧文字
    this.ctx.font = 'bold 24px YaHei-Bold';
    const _name = getLimitWords(it.Equipment.Name, 20, this.TitleRightWidth - 10, this.canvas || undefined);
    const _wordWidth = getWordsWidth(_name, 20, this.canvas || undefined);
    const _d = (this.TitleRightWidth - _wordWidth) / 2;
    // eslint-disable-next-line no-nested-ternary
    this.ctx.fillText(_name, x + 10 + 0.5 + this.layout.TitleLeftWidth + (_d ? (_d - 5 > 0 ? _d - 5 : 0) : 0), y + 14 + 0.5, this.TitleRightWidth);
    this.ctx.closePath();
  }
        ...
        it.TaskList.forEach((task) => {
          ...
          if (this.ctx && task._points && task._points.len >= 30) {
            ...
            const words = getLimitWords(
              `ID:${task.Code} ${task.Material || ''} ${(task.Number || task.Number === 0) ? task.Number : ''}${task.Unit || ''} ${task.Duration}`,
              12,
              task._points.len,
