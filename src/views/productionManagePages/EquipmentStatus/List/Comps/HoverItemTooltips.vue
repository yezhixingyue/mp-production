<template>
  <transition name="el-fade-in">
    <div class="hover-item-tooltips " v-show="hoverItemInfo.data.item" ref="oBox">
        <!-- 设备类别和组展示 -->
        <p class="ft-12">
          <span>{{hoverItemInfo.data.item?.Equipment.ClassName}}</span>
          <i v-show="hoverItemInfo.data.item?.Equipment.ClassName"> - </i>
        ...
        <p v-show="hoverItemInfo.data.Task?.Duration">
          ...
          <span class="c">{{hoverItemInfo.data.Task?.Duration}}</span>
        </p>
    </div>
  </transition>
</template>
