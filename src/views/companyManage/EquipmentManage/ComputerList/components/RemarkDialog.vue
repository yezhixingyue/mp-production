<template>
  <DialogContainerComp class="dialog" :width="512" top="15vh" :title="`设置电脑${operationTypeText}信息`"
    v-model:visible="localVisible" :danger="showDanger" :primaryText="showDanger ? operationTypeText : undefined"
    @open="open" @submit="submit" @danger="submit" @cancel="cancel">
    <el-form :model="ruleForm" ref="formRef" class="form" label-width="85px" style="margin-right: 10px;" size="small"
      v-if="ruleForm" @submit.prevent>
      <!-- 输入框 -->
      ...
      <!-- 警告 -->
      <el-form-item v-if='showDanger' style="margin-bottom: -24px;line-height: 20px;margin-top: 30px;">
        <span class="danger">
      ...
      <!-- 使用状态: 是否已启用 -->
      <el-form-item label="使用状态:" class="use-state" prop="state" v-if="operationType === EquipmentHistoryTypeEnum.callin" style="height: 48px;">
        <!-- 设置使用状态 -->
        <el-col :span="6">
          <el-form-item>
        ...
        <el-col :span="18">
          <el-form-item required label="使用人" prop="user"
            ...
            <el-select v-model="ruleForm.user" filterable style="width: 150px;top: 1px;">
...
const submit = async () => {
  ...
  if (bool) {
    const data: IRemarkSubmitParams = {
      ...
      creator: useUserStore().user?.StaffID || '',
